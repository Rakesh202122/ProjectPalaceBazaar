            ...
            <ModalBody p='16'>
                <Grid templateColumns={['1fr','3fr 1fr']}>
                    <Box px={['0','16']}>
                        ...
                        {
                            lectures.map((item, i)=>(
                                <VideoCard key={i} title={item.title} description={item.description} num={i+1} lectureId={item._id} courseId={id} deleteButtonHandler={deleteButtonHandler} loading={loading} />
                            ))
                        }
                    </Box>
                    <Box>
                        <form onSubmit={e=>addLectureHandler(e, id, title, description, video)}>
                            <VStack spacing={'4'}>
                                ...
                                <Input accept='video/mp4' required type={'file'} focusBorderColor='purple.300' css={{"&::file-selector-button":{...fileUploadCss, color:'purple'}}} onChange={ChangeVideoHandler}/>8
                                {
                                    videoPrev && (
